...
A successful instance artifact remains live until Packer destroys it. Set `keep_instance = true` to prevent artifact destruction and failed-build cleanup from deleting the build instance. OS volume builds delete their temporary instance after capture unless `keep_instance` is enabled.
...
- `artifact_volume_location_code` (string) - Deprecated: use artifact_volume_location_codes. Target location for a cloned OS volume artifact.

- `artifact_volume_location_codes` ([]string) - Target locations for cloned OS volume artifacts. The first location is primary; the effective locations also include the source location exactly once.

- `skip_shutdown_before_artifact` (bool) - Skip shutting down the instance before creating an OS volume artifact.

- `keep_instance` (bool) - Keep the build instance during failed-build cleanup and when Packer destroys the returned instance artifact.
...
Set `is_spot = true` to create the Packer build instance as Verda spot capacity. This defaults `contract` to `SPOT`. Setting `contract = "SPOT"` is also accepted and sends the same spot request to Verda. Configurations that set `is_spot = true` with a non-`SPOT` contract are rejected during `packer validate`.
...
Spot builds can also set discontinuation behavior for volumes created with the instance. Use `os_volume_spot_behavior` for the OS volume and `on_spot_discontinue` inside each `volume` block. Valid values are `keep_detached`, `move_to_trash`, and `delete_permanently`.

## OS Volume Artifacts

Set `artifact_type = "os_volume"` to return a cloned OS volume instead of the instance. The effective artifact locations are the configured `artifact_volume_location_codes` followed by the source instance location when it was not already listed, compared case-insensitively. The first configured location is the primary artifact ID, while console output and generated data expose every location and volume ID.

Verda does not support cloning an attached OS volume directly across datacenters. The builder first creates the source-location artifact, waits for it to become detached, and uses that artifact as the source for the remaining locations. The source-location artifact is included once in the effective artifact set.
...
### Persisting volume IDs

The Packer artifact ID is singular, so use a manifest post-processor to persist the complete multi-location mapping. Manifest custom data values are strings; encode the list and map as JSON.
...
Packer appends repeated runs to the manifest. For a single-build workflow, inspect the latest entry:

```sh
jq '.builds[-1].custom_data.VolumeIDsByLocation | fromjson' packer-manifest.json
```
...
For multiple locations, select the volume ID matching each instance location. The manifest stores the map as a JSON string inside its JSON document, so decode both layers. For multiple instances in the same location, clone that location's Packer artifact once per instance before passing each clone ID as `image`.

```hcl
locals {
  packer_manifest = jsondecode(file("${path.module}/packer-manifest.json"))
  packer_build    = local.packer_manifest.builds[length(local.packer_manifest.builds) - 1]

  os_volume_ids_by_location = jsondecode(
    local.packer_build.custom_data.VolumeIDsByLocation
  )
}

resource "verda_instance" "app" {
  instance_type = "1B200.30V"
  image         = local.os_volume_ids_by_location["FIN-03"]
  hostname      = "app-01"
